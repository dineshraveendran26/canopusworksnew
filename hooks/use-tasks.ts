...
export function useTasks() {
  ...
  const fetchTasks = useCallback(async () => {
    // Declare emergencyTimeout outside try block so it's accessible in finally
    ...
    try {
      ...
      // Clear the emergency timeout to prevent it from firing
      if (emergencyTimeout) {
  ...
  }, [user?.id]) // Only depend on user.id, fetchTasks is stable
