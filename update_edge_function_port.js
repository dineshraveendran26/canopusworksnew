...
// Create the updated Edge Function content with correct port
  ...
  try {
    ...
    try {
      ...
      const { data: resetData, error: resetError } = await supabase.auth.admin.generateLink({
        ...
        email: userEmail,
        options: {
          redirectTo: \`\${Deno.env.get('NEXT_PUBLIC_SITE_URL') || 'http://localhost:3003'}/auth/reset-password\`
        }
      });
      ...
      if (!resetError && resetData.properties.action_link) {
        ...
        if (tokenHash) {
          ...
          resetLink = \`\${Deno.env.get('NEXT_PUBLIC_SITE_URL') || 'http://localhost:3003'}/auth/reset-password?token_hash=\${tokenHash}&type=recovery\`;
...
fs.writeFileSync('edge-function-updated-port.ts', updatedEdgeFunction);

console.log('✅ Updated Edge Function for port 3003');
console.log('🔧 Changes made:');
console.log('   - Updated fallback URL from localhost:3002 to localhost:3003');
console.log('   - Maintained PKCE flow support');
console.log('   - Enhanced logging for debugging');
console.log('');
console.log('📁 Updated function saved to: edge-function-updated-port.ts');
