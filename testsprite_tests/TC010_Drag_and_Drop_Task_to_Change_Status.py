...
async def run_test():
    ...
    try:
        ...
        # Drag the task card 'Final Full function check task' from 'To Do' column to 'In Progress' column.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/main/div[2]/div/div[2]/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try to click on the task card to see if there is an option to change status manually or via context menu.
        frame = context.pages[-1]
